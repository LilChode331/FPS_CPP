...
void AFPS_CPPCharacter::Tick(float DeltaTime)
{
	...
	//GEngine->AddOnScreenDebugMessage(-1, 5, FColor::Cyan, TEXT("Hello World"));

	FVector lineStart = GetActorLocation();
	FVector lineEnd = lineStart + (GetActorForwardVector() * lineLenght);
	FVector sensorLine = sensor->GetActorLocation();
	FVector toSensor = sensorLine - lineStart;

	

	DrawDebugLine(GetWorld(), lineStart, lineEnd, FColor::Purple);
	DrawDebugLine(GetWorld(), lineStart, sensor->GetActorLocation(), FColor::Red);
	float dotProduct = FVector::DotProduct(GetActorForwardVector(), toSensor.GetSafeNormal());

	if (dotProduct >= threshHold)
	{
...
void AFPS_CPPCharacter::SetHasRifle(bool bNewHasRifle)
{
	bHasRifle = bNewHasRifle;
}

bool AFPS_CPPCharacter::GetHasRifle()
{
	return bHasRifle;
}