...
void UTP_WeaponComponent::DrawLine()
{
	...
	if (GetWorld()->LineTraceSingleByChannel(hit, startLocation, endLocation, ECC_Visibility, params))
	{
		...
		FVector incident = -GetOwner()->GetActorRightVector();
		
		FVector reflection = -(incident - 2 * FVector::DotProduct(incident, normal) * normal);
		reflection.Normalize();

		FVector reflectStart = impact;
		FVector reflectEnd = impact + reflection * 2000;

		DrawDebugLine(GetWorld(), impact, reflectEnd, FColor::Magenta);

		GEngine->AddOnScreenDebugMessage(-1, 0.1f, FColor::Red, TEXT("Hit Something: " + hit.GetActor()->GetName()));
